...
from ..constants import (
    VALID_URL_SCHEMES, EXCLUDED_URL_EXTENSIONS, 
    DEFAULT_TIMEOUT_MS, MAX_FILENAME_LENGTH
)
...
def validate_file_path(path: str, must_exist: bool = True, must_be_readable: bool = True) -> Tuple[bool, str]:
    """
...
def validate_output_directory(path: str, create_if_missing: bool = False) -> Tuple[bool, str]:
    """
...
def validate_model_name(model: str) -> Tuple[bool, str]:
    ...
    if "/" not in model:
        return False, "Model name should include provider (e.g., 'openrouter/model-name')"
    ...
    if not valid_pattern.match(provider) or not valid_pattern.match(model_name.replace("/", "")):
        return False, "Model name contains invalid characters (use only letters, numbers, dots, hyphens, underscores)"
    
    return True, ""


def validate_timeout(timeout_ms: int) -> Tuple[bool, str]:
    """
    ...
    # Minimum 1 second
    if timeout_ms < 1000:
        return False, "Timeout must be at least 1000ms (1 second)"
    
    # Maximum 10 minutes (reasonable upper bound)
    if timeout_ms > 600000:
        return False, "Timeout cannot exceed 600000ms (10 minutes)"
...
def validate_start_line(start_line: int, total_lines: int = None) -> Tuple[bool, str]:
    """
    ...
    if not isinstance(start_line, int):
        return False, "Start line must be an integer"
    
    if start_line < 1:
        return False, "Start line must be 1 or greater (1-based indexing)"
