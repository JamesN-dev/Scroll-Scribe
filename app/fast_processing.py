...
def get_fast_crawler_config(args) -> CrawlerRunConfig:
    ...
    return CrawlerRunConfig(
        ...
        excluded_tags=["script", "style", "nav", "footer", "aside", "header"],  # Remove noise
...
async def process_urls_fast(
    ...
    header_text = Text("⚡ Mode: Fast HTML→Markdown (No LLM)", justify="center", style="#f6c177")
    live_group = Group(header_rule, header_text, progress)
    
    logger.info(f"Starting FAST crawl for {len(urls_to_scrape)} URLs (no LLM processing)...")
    success_count: int = 0
    failed_count: int = 0
                ...
                for loop_index, (url, result) in enumerate(
                    ...
                    try:
                        if result.success and result.markdown:
                            ...
                            logger.error(f"Fast processing failed for {url}: {error_msg}")
                        ...
                        failed_count += 1
                        logger.error(f"Unexpected error in fast processing {url}: {exc}")
